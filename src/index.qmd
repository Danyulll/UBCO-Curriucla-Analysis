...
Slim introduces the concept of *efficient curricula* as a way to design curricula such that the most crucial courses are placed as early in a curriculum as possible [-@slim2016curricular]. The idea behind doing this is it will minimize the possibility of students not graduating on time as courses with the highest combinations of delay and blocking factors will completed first. To solve this problem an optimization model dubbed *Cruciality-Based Curriculum Balancing* (CBCB) utilizes a mix of *constraint satisfaction* and *integer linear programming* (ILP) techniques.

### Solving ILPs in R

We introduce a common way for solving ILPs in R based on this [guide](http://rtutorial.altervista.org/lp_solvers.html). Here is how we would solve the simple linear program

$$
\begin{align*}
\text{maximizie } 2x_1 + 4x_2 + 3x_3 & \\
\text{subject to } 3x_1 + 4x_2 + 2x_3 & \leq 60 \\
2x_2 + x_2 + 2x_3 &\leq 40 \\
...
# Define direction of inequality for each constraint
dir <- c("<=", "<=", "<=")

# Define the right hand side of constraints
rhs <- c(60, 40, 80)

# Solve the linear porgram. Set max to true for a maximization problems

ans <- Rglpk_solve_LP(obj, mat, dir, rhs, max = TRUE)

sprintf(
  "Optimum value of %f attained at (%f,%f,%f)",
  ans$optimum,
  ans$solution[1],
  ans$solution[2],
  ans$solution[3]
...
### Solving CSPs in R
*Constraint Satisfaction Problems* (CSPs) are solved using a variety of techniques here we go through an example of solving CSPs in R

```{r Example CSP Solve in R}
...
# Set initial values
x0 <- c(1, 2, 3, 4, 5)  # Initial values for l1 to ln

# Use optim to minimize the objective function
result <- optim(x0, objective, method = "L-BFGS-B")

# Get the minimized value of l
minimized_l <- result$value
```
...
[@slim2021restructuring] further showcases how one may find latent nodes and investigate how the likelihood of the model changes with these hidden nodes. We attempt this in our analysis.

**TO DO figure out if you have to introduce your own latent variables or if there is a way to automatically learn them**
...
## Analysis of UBCO Data Science Curriculum
...
Examining our maximally complex graph we can learn the set of key courses that are contributing most to our metrics. The total structural complexitity is 194, the total blocking factor is 80, and the total delay factor is 114.
...
### Improving Curricula Structure

#### Implementation of Patterns

#### Efficient Curricula and Comparison Through Simulation

As outlined in [@slim2016curricular] we may use  Monte Carlo simulations to see if the different structural complexities correlate to different graduation rates. We can then use constraint satisfication and integer linear programming to generate efficient curricula and see if graduation rates improve.

**TODO get access to CASL or make it yourself**

**TODO then run simulations on max and min curriculum as see what the difference is**

##### ILP to Generate Efficient Curricula

**TODO use ILP to get an efficient curriculum and see how they compare to max and min graphs through simulations**

```{r ILP to Generate Efficient Curriculum}
library(Rglpk)
```

##### Constraint Satisfaction to Generate Efficient Curricula

**TODO use constraint satisfaction to get an efficient curriculum and see how they compare to max and min graphs through simulations**

#### Bayesian Networks for Latent Node Discovery and GPA Prediction
